...
const ANCHORS = [{ id: 'slides', label: 'Slides' }, { id: 'leccion', label: 'Leccion' }, { id: 'lab', label: 'Lab' }, { id: 'artefactos', label: 'Artefactos' }, { id: 'dod', label: 'Definition of Done' }]
...
export default function Class07() {
  ...
  return <>
    ...
    <section className="section container" id="slides"><Reveal><h2 className="stitle">Presentacion de la clase</h2><p className="lede section-intro">25 diapositivas · contratos PCM, ASR, TTS, relay, fallos y medicion.</p><SlidesEmbed src={`${BASE}/slides.html`} title="Slides de la Clase 7: Speech Systems" previewLabel="Speech Systems" hint={25} /></Reveal></section>
    <section className="section container" id="leccion"><Reveal><h2 className="stitle">Leccion canonica</h2><p className="lede section-intro">PCM, ASR Whisper/Parakeet, TTS y un relay de voz con metricas, cancelacion y backpressure.</p><ClassLessonLinks base={BASE} classIndex={7} /><div className="grid grid--3">{CLASS07.outcomes.map(o => <FeatureCard key={o.tag} tag={o.tag} title={o.title} body={o.body} />)}</div></Reveal></section>
    ...
    <ClassArtifactsSection><div className="grid grid--2"><div className="card artifact"><span className="tag">EXAMPLES</span><div><h3>Ejemplos incrementales</h3><p className="artifact-desc">El lab incluye la secuencia ASR → TTS → relay y una prueba de formato.</p></div></div><div className="card artifact"><span className="tag">CHALLENGE</span><div><h3>Relay defendible</h3><p className="artifact-desc">Disena ventanas, backpressure, cancelacion y metricas reproducibles.</p><p><MarkdownLink staticPath={`${BASE}/challenge/challenge.md`}>challenge/challenge.md</MarkdownLink> · <MarkdownLink staticPath={`${BASE}/solution/solution.md`}>solution/solution.md</MarkdownLink></p></div></div><div className="card artifact"><span className="tag">ASSESSMENT</span><div><h3>Checkpoint + rubric</h3><p><MarkdownLink staticPath={`${BASE}/assessment/checkpoint.md`}>checkpoint.md</MarkdownLink> · <MarkdownLink staticPath={`${BASE}/assessment/rubric.md`}>rubric.md</MarkdownLink></p></div></div><div className="card artifact"><span className="tag">INSTRUCTOR</span><div><h3>Guia + NotebookLM</h3><p><MarkdownLink staticPath={`${BASE}/instructor/instructor-guide.md`}>instructor guide</MarkdownLink> · prompts en <MarkdownLink staticPath={`${BASE}/notebooklm/podcast-prompt.md`}>notebooklm/</MarkdownLink></p></div></div></div></ClassArtifactsSection>
    <section className="section container" id="dod"><Reveal><h2 className="stitle">Definition of Done</h2><p className="lede section-intro">Marca cada evidencia cuando la tengas.</p><DodLiveRegion message={feedback} /><ul className="dod">{CLASS07.dodItems.map(item => <li key={item.id}><label><input type="checkbox" checked={!!state[item.id]} onChange={e => toggle(item.id, e.target.checked)} /><span>{item.label}</span></label></li>)}</ul><div className="block example dod-next"><div className="block-title">Siguiente parada</div><div className="block-body"><Link to="/curriculum">Continua con el siguiente capitulo de Beyond Text. →</Link></div></div></Reveal></section>
    <div className="container class-back"><Link to="/curriculum">← Curriculo completo</Link></div>
  </>
}
