...
# Parte XII — Ruta de estudio guiada y práctica deliberada

Esta ruta convierte el texto en trabajo verificable. No persigue memorizar una lista de APIs:
en cada sesión el estudiante debe producir un artefacto que permita comprobar qué cambió, con
qué corpus y qué evidencia lo respalda. Las lecturas marcadas con `[W..]` se encuentran al final
del módulo; las capacidades de embeddings y RAG se limitan a las documentadas por QVAC. [W01]
[W02] [W03]

## Semana 1 — Entender y medir retrieval antes de generar

| Sesión | Estudia | Produce | Criterio de salida |
|---|---|---|---|
| 1 | Partes III, IV §§1–15 | Glosario propio de embedding, norma, cosine, Top-K, P@K, R@K y MRR | Explica por qué similitud no equivale a soporte factual. |
| 2 | §§16–24 | Corpus pequeño versionado y 12 preguntas con evidence esperada | Cada pregunta tiene al menos un chunk relevante etiquetado. |
| 3 | Partes III–IV | Script que calcula cosine con chequeo de dimensión y vector cero | Compara ranking por dot product y cosine sin declarar ganador universal. |
| 4 | Parte IV | Tabla de resultados para K=1, 3 y 5 | Reporta Precision@K, Recall@K y MRR por separado. |
| 5 | §§20–24 | Cinco hard negatives: negación, fecha, cantidad, entidad y paráfrasis | El estudiante explica qué error del ranking revela cada caso. |

**Checkpoint de la semana 1.** Sin usar un LLM generador, muestra una query, el embedding de la
query, el Top-K, los IDs de fuente y la razón por la que cada hit sí o no es evidencia. Si no se
puede inspeccionar la lista de resultados, todavía no es momento de construir RAG.

## Semana 2 — Convertir documentos en una base de conocimiento

| Sesión | Estudia | Produce | Criterio de salida |
|---|---|---|---|
| 6 | Parte V §§25–29 | Tres configuraciones: fixed-size, párrafo y una alternativa | Compara cobertura, duplicados y límites de cada configuración. |
| 7 | Parte V §§30–36 | Registro de extracción de PDF, tabla o código y sus errores | Distingue un problema de extracción de uno de chunking. |
| 8 | Parte VI §§37–43 | Workspace/índice con manifiesto: corpus, modelo, dimensión y chunk policy | Puede reconstruir qué versión produjo cada vector. |
| 9 | Parte VI §§44–48 | Prueba de lifecycle: close, reopen, update y delete | Verifica con búsquedas, no solo con ausencia de excepciones. |
| 10 | Partes V–VI | Comparativa de baseline contra alternativa sobre el mismo test set | Declara ganancia, pérdida o resultado inconcluso con ejemplos. |

**Regla de decisión.** Una estrategia de chunking no se adopta porque suena más sofisticada. Se
adopta cuando mejora una métrica o un caso de uso definido sin empeorar de forma inaceptable
latencia, costo, diversidad de evidencia o mantenimiento. Los trabajos de chunking citados en el
módulo se usan como hipótesis de investigación, no como recetas universales. [W15] [W16] [W18]

## Semana 3 — Generación grounded, evaluación y límites

| Sesión | Estudia | Produce | Criterio de salida |
|---|---|---|---|
| 11 | Parte VII §§49–55 | Prompt con bloques separados para política, evidencia y pregunta | Cada afirmación material de la respuesta apunta a un chunk visible. |
| 12 | Parte VII §§56–60 | Matriz de fallos: retrieval, generation, stale evidence y source conflict | Propone una corrección distinta para cada tipo de fallo. |
| 13 | Parte VIII §§61–65 | Test set congelado y reporte de regresión | Separa tuning de evaluación final. |
| 14 | Parte VIII §§66–70 | Unknown Knowledge Test y revisión humana de una muestra | Mide abstención y citation support; no solo calidad de redacción. |
| 15 | Partes IX–XI | Demo offline con trazabilidad, informe y limitaciones | Puede explicar el data path completo y sus fronteras de privacidad. |

## Protocolo para cada sesión

1. Formula una hipótesis falsable: por ejemplo, “chunking por párrafos mejora Recall@3 para las
   preguntas que dependen de una excepción de política”.
2. Cambia una sola variable principal; conserva corpus, query set, modelo y métrica cuando no son
   objeto del experimento.
3. Guarda la configuración, snapshot, IDs de resultados y tiempos por etapa.
4. Inspecciona al menos un éxito y un fracaso. Un promedio no sustituye el examen de evidencia.
5. Escribe una conclusión con alcance: qué corpus, qué queries y qué configuración cubre. Evita
   convertir un resultado local en una regla general.

## Plantilla de bitácora de estudio

```text
Fecha / sesión:
Pregunta o hipótesis:
Corpus snapshot y fuente:
Modelo de embeddings / versión:
Chunk policy / métrica / Top-K:
Resultado medido:
Un hit que sí soporta la respuesta:
Un hit que no la soporta o un fallo observado:
Interpretación:
Siguiente experimento:
Riesgo de privacidad o lifecycle observado:
```

La bitácora es parte del aprendizaje: permite distinguir una intuición de una conclusión que otra
persona puede revisar. Al terminar la ruta, el proyecto final de la Parte XI debe reutilizar los
artefactos reales producidos aquí, no inventar resultados retrospectivamente.

# Parte XIII — Checklist de afirmaciones que el alumno debe poder defender
